...
def get_library_internal_references(library_path, exported_symbols):
    ...
    """
    try:
        result = subprocess.run(['objdump', '-R', library_path], capture_output=True, text=True, check=True)
    except subprocess.CalledProcessError as e:
        print(f'Warning: Failed to run objdump on {library_path}: {e}', file=sys.stderr)
        return set()
    except FileNotFoundError:
        print(
            "Warning: 'objdump' command not found. Internal references won't be detected.", file=sys.stderr
        )
        return set()
...
def main():
    parser = argparse.ArgumentParser(description='Find unused exported symbols in a shared library')
    parser.add_argument(
        'library',
