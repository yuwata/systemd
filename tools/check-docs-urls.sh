...
# check-docs-urls.sh
# Extract external URLs from docs/ using rg, clean them, de-duplicate,
...
command -v rg >/dev/null 2>&1 || { echo "ERROR: ripgrep (rg) not found in PATH" >&2; exit 2; }
...
rg -o --hidden --no-ignore 'https?://[^\s<>"\)\]']+' docs \
    | sed -E 's/[)\]>,;:\"\'\)]*$//' \
    | sed -E 's/<[^>]*>//g' \
    | sort -u > "$OUT_LIST"
