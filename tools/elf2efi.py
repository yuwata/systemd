    ...
    for elf_seg in elf.iter_segments():
        if elf_seg['p_type'] == 'PT_LOAD' and elf_seg['p_align'] != SECTION_ALIGNMENT:
            raise BadSectionError(
                f"ELF segment {elf_seg['p_type']} is not properly aligned"
...
def convert_sections(elf: ELFFile, opt: PeOptionalHeader) -> list[PeSection]:
    ...
    for pe_s in iter_copy_sections(elf):
        ...
        if pe_s.VirtualAddress < sum(last_vma):
            raise BadSectionError(
                f'Section {pe_s.Name.decode()!r} @0x{pe_s.VirtualAddress:x} overlaps'
...
def convert_elf_reloc_table(
    ...
    for reloc in elf_reloc_table.iter_relocations():
        ...
        if reloc['r_info_type'] == RELATIVE_RELOC:
            apply_elf_relative_relocation(reloc, elf_image_base, sections, elf.elfclass // 8)
...
def convert_elf_relocations(
    ...
    for reloc_type, reloc_table in dynamic.get_relocation_tables().items():
        ...
        convert_elf_reloc_table(elf, reloc_table, opt.ImageBase + segment_offset, sections, pe_reloc_blocks)
...
def write_pe(
    ...
    for pe_s in sorted(sections, key=lambda s: s.VirtualAddress):
        if pe_s.VirtualAddress < opt.SizeOfHeaders:
            raise BadSectionError(
                f'Section {pe_s.Name} @0x{pe_s.VirtualAddress:x} overlaps'
