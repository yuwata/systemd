...
static int address_is_ready_to_configure(Link *link, const Address *address, Address **ret) {
        int r;
        ...
        if (address_get(link, address, ret) < 0) {
                ...
                r = address_add(link, address, ret);
                if (r < 0)
                        return r;
        }
...
int request_process_address(Request *req) {
        ...
        r = address_is_ready_to_configure(link, req->address, &a);
        if (r <= 0)
                return r;
