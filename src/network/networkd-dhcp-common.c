...
int config_parse_dhcp_send_option(
        ...
        OrderedHashmap **dhcp6_options = ltype == AF_INET6 ? ASSERT_PTR(data) : NULL;
        TLV *dhcp4_options = ltype == AF_INET ? ASSERT_PTR(data) : NULL;
        ...
        if (ltype == AF_INET6) {
                ...
                r = tlv_append(dhcp4_options, u8, sz, udata);
                if (r < 0) {
                        log_syntax(unit, LOG_WARNING, filename, line, r,
                                   "Failed to store DHCP option '%s', ignoring assignment: %m", rvalue);
                        return 0;
