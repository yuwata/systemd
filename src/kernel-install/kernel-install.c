...
static int context_get_rfd_for_command_line(const Context *c) {
        assert(c);
        /* The path specified via command line should be relative to CWD unless --root/--image specified. */
        return arg_root ? c->rfd : AT_FDCWD;
}
...
static int context_set_kernel(Context *c, const char *s) {
        assert(c);
        return context_set_path(c, context_get_rfd_for_command_line(c), s, "command line", "kernel image file", &c->kernel);
}
...
static int context_set_initrds(Context *c, char* const* strv) {
        assert(c);
        return context_set_path_strv(c, context_get_rfd_for_command_line(c), strv, "command line", "initrds", &c->initrds);
}
...
static int context_execute(Context *c) {
        ...
        if (DEBUG_LOGGING) {
                ...
                _cleanup_free_ char *z = strv_join_full(strv_skip(c->argv, 1), " ", arg_root, /* escape_separator = */ false);
