...
static const int service_type = IPTOS_CLASS_CS3;
...
static void setup(sd_event_io_handler_t io_handler, sd_dhcp_client_callback_t client_handler, sd_dhcp_client **ret) {
        ...
        ASSERT_OK(sd_dhcp_client_set_service_type(client, service_type));
        ...
        STRV_FOREACH(s, vendor_specific_strv)
                ASSERT_OK(dhcp_options_append(&vendor_specific, ++c, strlen(*s), *s));
        ...
        _cleanup_hashmap_free_ Hashmap *extra_options = NULL;
        ASSERT_OK(dhcp_options_append(&extra_options, 163, strlen(extra_option_163), extra_option_163));
        ASSERT_OK(dhcp_options_append(&extra_options, 164, strlen(extra_option_164), extra_option_164));
        ASSERT_OK(dhcp_client_set_extra_options(client, extra_options));

        /* IO event source for fake server side */
...
static void verify_request(sd_dhcp_message *m, uint8_t type) {
        ...
        _cleanup_hashmap_free_ Hashmap *vendor_specific = NULL;
        ASSERT_OK(dhcp_message_get_option_vendor_specific(m, &vendor_specific));
        ASSERT_EQ(hashmap_size(vendor_specific), strv_length((char**) vendor_specific_strv));
        uint8_t c = 0;
        STRV_FOREACH(s, vendor_specific_strv) {
                sd_dhcp_option *o = hashmap_get(vendor_specific, UINT_TO_PTR(++c));
                ASSERT_EQ(memcmp_nn(o->data, o->length, *s, strlen(*s)), 0);
        }
        ...
        ASSERT_OK(dhcp_message_get_option_user_class(m, &user_class));
...
static void verify_anonymized_request(sd_dhcp_message *m, uint8_t type) {
        ...
        ASSERT_ERROR(dhcp_message_get_option_u16(m, SD_DHCP_OPTION_MAXIMUM_MESSAGE_SIZE, /* ret= */ NULL), ENODATA);
        ASSERT_ERROR(dhcp_message_get_option_string(m, SD_DHCP_OPTION_MUD_URL, /* ret= */ NULL), ENODATA);
        ASSERT_ERROR(dhcp_message_get_option_hostname(m, /* ret= */ NULL), ENODATA);
        ASSERT_ERROR(dhcp_message_get_option_string(m, SD_DHCP_OPTION_VENDOR_CLASS_IDENTIFIER, /* ret= */ NULL), ENODATA);
        ASSERT_ERROR(dhcp_message_get_option_vendor_specific(m, /* ret= */ NULL), ENODATA);
        ASSERT_ERROR(dhcp_message_get_option_user_class(m, /* ret= */ NULL), ENODATA);
        ASSERT_ERROR(dhcp_message_get_option_string(m, 163, /* ret= */ NULL), ENODATA);
        ASSERT_ERROR(dhcp_message_get_option_string(m, 164, /* ret= */ NULL), ENODATA);
}
...
static int ipv6_only_offer_io_handler(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        sd_dhcp_client *client = ASSERT_PTR(userdata);
        static unsigned count = 0;
...
static int ipv6_only_early_client_handler(sd_dhcp_client *client, int event, void *userdata) {
        sd_event *e = ASSERT_PTR(userdata);
        static unsigned count = 0;
...
static int ipv6_only_selecting_client_handler(sd_dhcp_client *client, int event, void *userdata) {
        sd_event *e = ASSERT_PTR(userdata);
        static unsigned count = 0;
...
static int ipv6_only_before_ack_io_handler(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        sd_dhcp_client *client = ASSERT_PTR(userdata);
        static unsigned count = 0;
...
static int ipv6_only_before_ack_defer_handler(sd_event_source *s, void *userdata) {
        sd_dhcp_client *client = ASSERT_PTR(userdata);
...
static int ipv6_only_before_ack_client_handler(sd_dhcp_client *client, int event, void *userdata) {
        ...
        case 1:
                ...
                ASSERT_OK(sd_event_add_defer(e, /* ret= */ NULL, ipv6_only_before_ack_defer_handler, client));
                break;
        case 2:
                ASSERT_EQ(event, SD_DHCP_CLIENT_EVENT_STOP);
                verify_reply(client, DHCP_STATE_REQUESTING);
...
static int ipv6_only_ack_io_handler(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        sd_dhcp_client *client = ASSERT_PTR(userdata);
        static unsigned count = 0;
...
static int ipv6_only_before_bound_defer_handler(sd_event_source *s, void *userdata) {
        sd_dhcp_client *client = ASSERT_PTR(userdata);

        if (ASSERT_OK(sd_event_source_get_enabled(client->timeout_ipv6_only_mode, /* ret= */ NULL))) {
                ASSERT_EQ(client->state, DHCP_STATE_REQUESTING);
                ASSERT_OK(sd_dhcp_client_set_ipv6_connectivity(client, true));
        } else
...
static int ipv6_only_before_bound_client_handler(sd_dhcp_client *client, int event, void *userdata) {
        sd_event *e = ASSERT_PTR(userdata);
        static unsigned count = 0;
        ...
        case 1:
                ...
                /* Boost the time for sending DHCPREQUEST */
                ASSERT_OK(sd_event_add_defer(e, /* ret= */ NULL, ipv6_only_before_ack_defer_handler, client));
                /* Set IPv6 connectivity after DHCPACK */
                ASSERT_OK(sd_event_add_defer(e, /* ret= */ NULL, ipv6_only_before_bound_defer_handler, client));
                break;
        case 2:
                ASSERT_EQ(event, SD_DHCP_CLIENT_EVENT_IP_ACQUIRE);
                verify_reply(client, DHCP_STATE_BOUND);
...
static int ipv6_only_exit_defer_handler(sd_event_source *s, void *userdata) {
        sd_dhcp_client *client = ASSERT_PTR(userdata);
...
static int ipv6_only_after_bound_defer_handler(sd_event_source *s, void *userdata) {
        sd_dhcp_client *client = ASSERT_PTR(userdata);

        ASSERT_EQ(client->state, DHCP_STATE_BOUND);
        ASSERT_OK(sd_dhcp_client_set_ipv6_connectivity(client, true));
        ASSERT_OK(sd_event_add_defer(sd_event_source_get_event(s), /* ret= */ NULL, ipv6_only_exit_defer_handler, client));
        return 0;
}

static int ipv6_only_after_bound_client_handler(sd_dhcp_client *client, int event, void *userdata) {
        sd_event *e = ASSERT_PTR(userdata);
        static unsigned count = 0;
        ...
        case 2:
                ASSERT_EQ(event, SD_DHCP_CLIENT_EVENT_IP_ACQUIRE);
                verify_reply(client, DHCP_STATE_BOUND);
                ASSERT_OK(sd_event_add_defer(e, /* ret= */ NULL, ipv6_only_after_bound_defer_handler, client));
                break;
        default:
                assert_not_reached();
        }
...
static int ipv6_only_init_reboot_io_handler(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        sd_dhcp_client *client = ASSERT_PTR(userdata);
        static unsigned count = 0;
...
static int ipv6_only_init_reboot_client_handler(sd_dhcp_client *client, int event, void *userdata) {
        ...
        case 1:
                ASSERT_EQ(event, SD_DHCP_CLIENT_EVENT_STOP);
                verify_reply(client, DHCP_STATE_REBOOTING);
                ASSERT_OK(sd_event_exit(e, 0));
                break;
        default:
                assert_not_reached();
...
static int ipv6_only_rapid_commit_io_handler(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        sd_dhcp_client *client = ASSERT_PTR(userdata);
        static unsigned count = 0;
...
TEST(ipv6_only) {
        ...
        setup(ipv6_only_offer_io_handler, ipv6_only_early_client_handler, &client);
        ...
        /* case 2: IPv6 connectivity is acquired just after client started. */
        setup(ipv6_only_offer_io_handler, ipv6_only_early_client_handler, &client);
        ASSERT_OK(sd_dhcp_client_set_request_option(client, SD_DHCP_OPTION_IPV6_ONLY_PREFERRED));
        ASSERT_OK(sd_dhcp_client_start(client));
        ASSERT_OK(sd_dhcp_client_set_ipv6_connectivity(client, true));
        ...
        /* case 3: IPv6 connectivity is acquired during selecting. */
        setup(ipv6_only_offer_io_handler, ipv6_only_selecting_client_handler, &client);
        ASSERT_OK(sd_dhcp_client_set_request_option(client, SD_DHCP_OPTION_IPV6_ONLY_PREFERRED));
        ASSERT_OK(sd_dhcp_client_start(client));
        ASSERT_OK(sd_event_loop(sd_dhcp_client_get_event(client)));
        ...
        /* case 4: IPv6 connectivity is acquired before sending DHCPREQUEST. */
        setup(ipv6_only_offer_io_handler, ipv6_only_before_request_client_handler, &client);
        ASSERT_OK(sd_dhcp_client_set_request_option(client, SD_DHCP_OPTION_IPV6_ONLY_PREFERRED));
        ASSERT_OK(sd_dhcp_client_start(client));
        ASSERT_OK(sd_event_loop(sd_dhcp_client_get_event(client)));
        ...
        /* case 5: IPv6 connectivity is acquired before DHCPACK received. */
        setup(ipv6_only_before_ack_io_handler, ipv6_only_before_ack_client_handler, &client);
        ASSERT_OK(sd_dhcp_client_set_request_option(client, SD_DHCP_OPTION_IPV6_ONLY_PREFERRED));
        ASSERT_OK(sd_dhcp_client_start(client));
        ASSERT_OK(sd_event_loop(sd_dhcp_client_get_event(client)));
        ...
        /* case 6: IPv6 connectivity is acquired before entring the bound state. In this case, IPv6 only mode is ignored. */
        setup(ipv6_only_ack_io_handler, ipv6_only_before_bound_client_handler, &client);
        ASSERT_OK(sd_dhcp_client_set_request_option(client, SD_DHCP_OPTION_IPV6_ONLY_PREFERRED));
        ASSERT_OK(sd_dhcp_client_start(client));
        ASSERT_OK(sd_event_loop(sd_dhcp_client_get_event(client)));
        ...
        /* case 7: IPv6 connectivity is acquired after entring the bound state. In this case, IPv6 only mode is ignored. */
        setup(ipv6_only_ack_io_handler, ipv6_only_after_bound_client_handler, &client);
        ASSERT_OK(sd_dhcp_client_set_request_option(client, SD_DHCP_OPTION_IPV6_ONLY_PREFERRED));
        ASSERT_OK(sd_dhcp_client_start(client));
        ASSERT_OK(sd_event_loop(sd_dhcp_client_get_event(client)));
        ...
        /* case 8: IPv6 connectivity is acquired during rebooting. */
        setup(ipv6_only_init_reboot_io_handler, ipv6_only_init_reboot_client_handler, &client);
        ASSERT_OK(sd_dhcp_client_set_request_option(client, SD_DHCP_OPTION_IPV6_ONLY_PREFERRED));
        ASSERT_OK(sd_dhcp_client_set_request_address(client, &client_address.in));
        ASSERT_OK(sd_dhcp_client_set_ipv6_connectivity(client, true));
        ASSERT_OK(sd_dhcp_client_start(client));
        ...
        /* case 9: IPv6 connectivity is acquired on rapid commit. */
        setup(ipv6_only_rapid_commit_io_handler, ipv6_only_rapid_commit_client_handler, &client);
        ASSERT_OK(sd_dhcp_client_set_rapid_commit(client, true));
        ASSERT_OK(sd_dhcp_client_set_request_option(client, SD_DHCP_OPTION_IPV6_ONLY_PREFERRED));
        ASSERT_OK(sd_dhcp_client_set_ipv6_connectivity(client, true));
        ASSERT_OK(sd_dhcp_client_start(client));
...
static int bootp_io_handler(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        case 1: {
                ...
                verify_header(request);
                ASSERT_TRUE(hashmap_isempty(request->options));
