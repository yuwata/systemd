...
int downstream_send_message(sd_dhcp_relay_interface *interface, sd_dhcp_message *message) {
         ...
         * where hlen == 0), we broadcast the reply. Otherwise, we unicast the reply to the client's hardware
         * address. */
        struct hw_addr_data hw_addr_from_message = {};
        if (!FLAGS_SET(be16toh(message->header.flags), 0x8000)) {
                r = dhcp_message_get_hw_addr(message, &hw_addr_from_message);
                if (r < 0)
                        return r;
        }
        ...
        struct hw_addr_data *hw_addr = NULL;
        if (!hw_addr_is_null(&hw_addr_from_message))
                hw_addr = &hw_addr_from_message;
        else if (message->header.htype == ARPHRD_ETHER || message->header.hlen == ETH_ALEN)
                hw_addr = &default_eth_bcast;
        else if (message->header.htype == ARPHRD_INFINIBAND)
                hw_addr = &default_ib_bcast;
        else
                return -ENXIO;
