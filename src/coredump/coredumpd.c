...
#define COREDUMP_SOCKET_MAX_CONNECTION 16
#define COREDUMP_KERNEL_SOCKET_PATH "/run/systemd/coredump-kernel.socket"
...
struct Manager {
        ...
        sd_event *event;
        sd_event_source *kernel_socket_event_source;
        int kernel_socket;
...
static Manager* manager_free(Manager *manager) {
        ...
        sd_event_source_unref(manager->kernel_socket_event_source);
        sd_event_unref(manager->event);
        safe_close(manager->kernel_socket);
...
static int manager_spawn_worker(Manager *manager, int coredump_fd) {
        ...
        if (r == 0) {
                ...
                r = coredump_worker_main(&manager->config, manager->request_mode, coredump_fd);
        ...
        r = sd_event_add_time_relative(manager->event, &worker->timer_event_source,
                                       CLOCK_MONOTONIC, 30 * USEC_PER_MINUTE, USEC_PER_SEC,
...
static int on_worker_notify(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        if (strv_contains(l, "PROCESSED=1")) {
                log_debug("Received notification from worker process ["PID_FMT"].", sender.pid);
                return 0;
        }
...
static int on_sigterm(sd_event_source *s, const struct signalfd_siginfo *si, void *userdata) {
        ...
        manager->kernel_socket_event_source = sd_event_source_disable_unref(manager->kernel_socket_event_source);

        r = sd_event_add_post(manager->event, /* ret = */ NULL, on_post, manager);
        if (r < 0)
                return sd_event_exit(manager->event,
                                     log_error_errno(r, "Failed to create post event source: %m"));
...
static int manager_init(Manager *manager) {
        bool socket_is_new = false;
        int r;
        ...
        if (manager->kernel_socket < 0) {
                ...
                r = sockaddr_un_set_path(&sa.un, COREDUMP_KERNEL_SOCKET_PATH);
                if (r < 0)
                ...
                manager->kernel_socket = TAKE_FD(fd);
                socket_is_new = true;

                /* First, try to use @@ socket, which is supported since kernel v6.17. */
                r = set_core_pattern("@@" COREDUMP_KERNEL_SOCKET_PATH);
                if (r >= 0)
                        manager->request_mode = true;
                if (r == -EINVAL) {
                        /* If not supported, let's try to use @ socket, which is supported since kernel v6.16. */
                        r = set_core_pattern("@" COREDUMP_KERNEL_SOCKET_PATH);
                        if (r >= 0)
                                manager->request_mode = false;
                }
        } else if (manager->request_mode)
                r = set_core_pattern("@@" COREDUMP_KERNEL_SOCKET_PATH);
        else
                r = set_core_pattern("@" COREDUMP_KERNEL_SOCKET_PATH);
        if (r == -EINVAL) {
                log_info("The kernel does not support socket coredump pattern, exiting.");
                return 0;
        }
        if (r < 0)
                return log_error_errno(r, "Failed to set kernel coredump pattern: %m");

        log_debug("kernel.core_pattern successfully written as \"%s" COREDUMP_KERNEL_SOCKET_PATH "\".",
                  manager->request_mode ? "@@" : "@");

        if (socket_is_new) {
                r = notify_push_fd(manager->kernel_socket,
                                   manager->request_mode ? "coredump-kernel-socket-request" : "coredump-kernel-socket");
                if (r < 0)
                        log_warning_errno(r, "Failed to push coredump socket to service manager, ignoring: %m");
                else
        ...
        _cleanup_(sd_event_source_unrefp) sd_event_source *s = NULL;
        r = sd_event_add_io(event, &s, manager->kernel_socket, EPOLLIN, on_connect, manager);
        if (r < 0)
                return log_error_errno(r, "Failed to allocate IO event source: %m");

        (void) sd_event_source_set_description(s, "coredump-kernel-socket-event");
        ...
        manager->kernel_socket_event_source = TAKE_PTR(s);
        return 1; /* initialized */
}

static int manager_set_kernel_socket(Manager *manager, int fd, bool request_mode) {
        int r;
        ...
        if (manager->kernel_socket >= 0)
                return log_error_errno(SYNTHETIC_ERRNO(EALREADY),
                                         "Received multiple coredump socket (%i), ignoring.", fd);

        r = sd_is_socket_unix(fd, SOCK_STREAM, /* listening= */ true, COREDUMP_KERNEL_SOCKET_PATH, /* length= */ 0);
        if (r < 0)
                return log_warning_errno(r, "Failed to check if fd (%i) is a unix socket, ignoring: %m", fd);
        if (r == 0)
...
static int manager_listen_fds(Manager *manager) {
        ...
        for (int i = 0; i < n; i++) {
                ...
                if (streq(names[i], "coredump-kernel-socket-request"))
                        r = manager_set_kernel_socket(manager, fd, /* request_mode= */ true);
                else if (streq(names[i], "coredump-kernel-socket"))
                        r = manager_set_kernel_socket(manager, fd, /* request_mode= */ false);
                else
...
int run_coredumpd(int argc, char *argv[]) {
        int r;
