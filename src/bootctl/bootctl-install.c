...
static int install_secure_boot_auto_enroll(const char *esp, X509 *certificate, EVP_PKEY *private_key) {
        ...
        int dercertsz;
        dercertsz = sym_i2d_X509(certificate, &dercert);
        if (dercertsz < 0)
        ...
        *siglist = (EFI_SIGNATURE_LIST) {
        ...
        FOREACH_STRING(db, "PK", "KEK", "db") {
                ...
                p7 = sym_PKCS7_sign(certificate, private_key, /*certs=*/ NULL, bio, PKCS7_DETACHED|PKCS7_NOATTR|PKCS7_BINARY|PKCS7_NOSMIMECAP);
                if (!p7)
