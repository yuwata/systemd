...
static bool interface_in_restricted_list(Manager *m, int ifindex) {
        ...
        if (!set_isempty(m->restrict_interfaces)) {
                char ifname[IF_NAMESIZE + 1];

                if (format_ifname_full(ifindex, ifname, FORMAT_IFNAME_IFINDEX)) {
                        SET_FOREACH(p, m->restrict_interfaces, j)
...
Manager *manager_free(Manager *m) {
        ...
        m->restrict_interfaces = set_free_free(m->restrict_interfaces);
